...
export const Optojscomplicated = (rator: PrimOp, rands: CExp[]): Result<string> =>
    rator.op === "number?" || rator.op === "boolean?" || rator.op === "symbol?" || rator.op === "string?" ? bind(l30ToJS(rands[0]), (rand: string) => makeOk(`${Optojs(rator.op)}(${rand})`)) :
    ...
    rator.op === "'" ? bind(l30ToJS(rands[0]), (rand: string) => makeOk(`(\"${rand}\")`)) :
    rator.op === "string=?" ? bind(mapResult(l30ToJS, rands), (rands: string[]) => makeOk(`(${rands[0]} === ${rands[1]})`)) :
                    bind(mapResult(l30ToJS, rands), (rands) => makeOk("(" + rands.join(" " + Optojs(rator.op) + " ") + ")"));
